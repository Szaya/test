...
public class BankAccount {
     ...
     */
    public void debit(double amount) throws AccountFrozenException, IllegalArgumentException {
        if (frozen) {
            throw new AccountFrozenException("Account frozen");
        }
        if (amount < 0){
            throw new IllegalArgumentException ("Debit's can't be negative!");
        }
        balance -= amount;
    ...
    public void credit(double amount) throws IllegalArgumentException {
        if (amount < 0){
            throw new IllegalArgumentException ("Credit's can't be negative!");
        }
        balance += amount;
